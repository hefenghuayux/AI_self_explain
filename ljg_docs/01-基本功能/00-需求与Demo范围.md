...
本阶段只验证学生提交最终文本之后，AI 能否正确评价并引导学生完成自讲。系统仍需实现语音录制和 ASR 转写；转写会直接回填到对应的可编辑输入框，学生修改后通过原有提交操作形成最终文本。AI 不以 ASR 原文作为学生是否讲对的依据。
...
6. 在无法可靠判断、学生申诉或发现其他需要人工处理的问题时，申请人工复核，但仍允许学生继续自动自讲。

## 3. 目标用户

### 3.1 学生

- 在同一个主输入框持续编写和修改题干理解、分析过程或完整自讲。
- 选择“提交自讲”“我不会，给我一点提示”或“我有疑问”。
- 通过语音或人工输入文本提交自讲内容。
- 使用语音时直接在对应输入框查看和修改 ASR 转写文本。
...
系统申请人工复核时，只保存并报告具体原因。本阶段不实现离线复核、派单、处理或复核后状态更新功能；申请不阻断学生继续自讲。

## 4. 核心使用场景

1. 人工录入题目和评分材料。
2. 学生在主输入框写入当前过程，并选择“提交自讲”“我不会，给我一点提示”或“我有疑问”。
3. 提交自讲时评价主输入框的不可变快照；请求提示时先生成子问题或回答简单疑问，系统按确定性规则计数和阈值检查。
4. 学生可以录制语音，由系统生成 ASR 转写；也可以直接人工输入文本。
5. 语音转写完成后直接回填到对应可编辑输入框；学生通过“提交自讲”“回答子问题”“我有疑问”或“AI 说错了”提交最终文本，不设置单独的语音确认阶段。
6. AI 只评价学生最终提交的自讲文本。
...
11. AI 无法可靠判断、学生提出申诉，或发生其他需要人工处理的问题时，申请人工复核并继续自动自讲；该次人工复核说明不计入有效支持次数。
...
- 主输入框的当前草稿，以及每次提交时保存的不可变自讲快照。
- 学生操作：“提交自讲”“我不会，给我一点提示”“我有疑问”“我不知道卡在哪里，给点提示”。
- “我有疑问”时的独立疑问文本和子问题答案。
- 原始音频，仅在语音输入时产生。
...
| `needHumanReason` | `correctness = UNCERTAIN` 时的具体原因；其他情况为空 |

`needHumanReason` 使用大模型生成的具体原因文本，不再拆分为转写质量、判断矛盾等独立标签。第一次模型调用只返回上述评价事实，不返回面向学生的反馈、下一动作或引导问题。后端根据合法评价和确定性规则计算教学动作；需要教学内容时，再调用独立教学模型生成 `content` 和受动作约束的 `questions`。

### 6.1.1 学生可见评价与反馈时间线

- 学生提交确认文本后，只展示中文化的正确性和完整性结果：`正确`、`有错误`、`暂无法可靠判断`、`完整`、`不完整`。
- 学生端不展示 `CORRECT`、`INCOMPLETE`、覆盖/缺失评分点、原始 JSON、结构校验错误或模型原始响应。
- 按实际发送顺序展示本会话全部学生可见 AI 反馈、聚焦追问、纠错、有效支持、完整解析展示事件和人工处理说明；刷新页面后仍可读取。
- 结构校验失败、模型重试和其他技术错误会记录审计信息，并以“已申请人工复核，你可以继续自讲”的系统说明进入学生反馈时间线；不展示技术错误细节。
...
`AI_UNCERTAIN` 不作为单独业务状态。评价模型返回 `correctness = UNCERTAIN` 时必须同时返回非空 `needHumanReason`，后端确定性进入人工处理分支。

`COMPLETED` 是唯一的学习成功终态；`STOPPED_LIMIT` 是唯一的非成功终态。人工复核申请是过程事件，不是业务状态。
...
1. 人工录入题目和评分材料。
2. 可持续编辑的主输入框、提交自讲、请求提示和提出疑问。
3. 语音录制、ASR 转写、转写直接回填和学生修改。
4. 人工直接输入自讲文本。
5. 对确认文本进行正确性和完整性评价。
6. 识别已覆盖和缺失的关键评分点。
...
13. 学生申诉后申请人工复核，不进行 AI 申诉复核，学生仍可继续自讲。
14. AI 无法可靠判断或认为必须人工处理时申请人工复核，并输出具体原因、暂定想法和继续自讲方向。
15. 暂停与恢复会话。
16. 保存完整测试过程和统计所需数据。
...
3. AI 评价对象是学生最终提交的文本。原始音频和 ASR 转写用于过程记录，不作为判定学生对错的直接依据。
4. `confidence` 在 Demo 阶段固定为 `1`，不能用于控制流程。
5. `CORRECT + COMPLETE` 才能进入 `COMPLETED`。
6. `UNCERTAIN` 必须返回非空 `needHumanReason`；系统确定性申请人工复核。
7. 学生申诉必须填写理由，提交后立即申请人工复核，不增加支持次数，且可继续自讲。
8. AI 可以根据上下文判断文本质量不足、前后判断矛盾或其他无法继续自动评价的情况，并输出具体的 `needHumanReason`；后端根据 `correctness = UNCERTAIN` 确定性申请人工复核，不调用教学模型，也不增加支持次数。
...
11.1 后端动作是 `ASK_FOCUSED_QUESTION` 或 `CORRECT_AND_ASK` 时，第二次教学模型调用必须返回恰好一个 `questions` 子问题；校验通过并实际发送后，保存为 `SupportEvent.guided_questions`，进入“回答子问题”输入块。
...
17. AI 输出结构或字段关系不合法时按配置有限重试；仍不合法则申请人工复核，保存原始响应和校验错误，并回到可继续自讲的稳定阶段。
18. AI 和 ASR 的超时、重试次数及重试间隔从配置读取；AI 重试耗尽后申请人工复核并回到可继续自讲的稳定阶段，ASR 重试耗尽后保持 `IN_PROGRESS` 并退回最近可重试的语音输入阶段。
19. Demo 只允许在等待学生操作的稳定阶段暂停；ASR 转写或 AI 评价处理中提出暂停时，先等待本次调用结束。
20. 学生端展示的状态、正确性、完整性、支持类型和完成结果必须使用中文文案；前端中文文案不得参与业务状态判断。
21. 本文件第 21 节对提示、疑问和主输入框的修订，在与前文存在冲突时优先适用。
...
| `guidedQuestions` | 可选的人工提示子问题列表，供模型组织子问题提示时优先参考 |
| `fullSolution` | 完整解析 |
...
| `confirmedText` | 学生修改后通过原有操作提交、或直接输入的最终文本 |
| `confirmedAt` | 最终提交时间 |

`confirmedText` 是 AI 评价的唯一学生表达输入。ASR 原文先回填为草稿，只有原有提交操作写入 `confirmedText`；空文本必须在进入 AI 评价前被拦截并提示学生重新提交。
...
| `needHumanReason` | `correctness = UNCERTAIN` 时必填，其他情况为空 |

评价输出禁止包含 `feedback`、`nextAction` 和 `guidedQuestions`，并使用 `extra = forbid` 拒绝额外字段。教学动作由后端根据 `correctness + completeness`、支持上限、无进展计数、解析展示和第二轮规则确定；需要生成教学内容时，独立教学模型只返回 `content` 和 `questions`。
...
- `correctness = UNCERTAIN` 时 `needHumanReason` 为空。
- `correctness != UNCERTAIN` 时返回了非空 `needHumanReason`。
- 返回了 `feedback`、`nextAction`、`guidedQuestions` 或其他额外字段。
- `ASK_FOCUSED_QUESTION` 或 `CORRECT_AND_ASK` 没有恰好一个独立子问题，或其他动作返回了子问题。
...
4. 重试仍失败时申请人工复核，保存原始响应、全部校验错误和人工处理原因，并回到可继续自讲的稳定阶段。
5. 结构重试失败不增加有效支持次数。
...
| `needHumanReason` | 最近一次人工复核申请的具体原因；用于审计，不作为终态标记 |
...
flowchart TD
    ...
    INPUT --> MODE{"输入方式"}
    MODE -- "语音" --> ASR["生成 ASR 转写并回填可编辑草稿"]
    ASR --> EDIT["学生修改后使用原有操作提交"]
    MODE -- "文本" --> EDIT["学生直接提交文本"]
    EDIT --> EVAL["AI 评价最终提交文本"]

    EVAL --> RESULT{"结构校验与确定性规则"}
    RESULT -- "正确且完整" --> COMPLETED["COMPLETED<br/>写入 completionType"]
    RESULT -- "继续学习" --> REPLY["展示聚焦追问、纠错或提示"]
    RESULT -- "需要人工处理" --> HUMAN_REVIEW["申请人工复核<br/>保存具体原因，不计支持"]
    HUMAN_REVIEW --> REPLY
...
| `CORRECT` | `INCOMPLETE` | `ASK_FOCUSED_QUESTION` | 创建一个独立子问题并进入“回答子问题” | 否 |
| `WRONG` | `COMPLETE` | `GIVE_CORRECTION` | 进入支持阈值检查 | 是 |
| `WRONG` | `INCOMPLETE` | `CORRECT_AND_ASK` | 先进入支持阈值检查；未到上限时创建一个独立子问题并进入“回答子问题” | 是，只计一次 |
| `UNCERTAIN` | 任意 | `NEED_HUMAN` | 保存原因，进入人工处理流程，不调用教学模型 | 否 |
...
学生在自动流程中只查看中文化的正确性、完整性和学生可见反馈时间线。`CORRECT + INCOMPLETE` 的教学内容只能围绕缺失评分点提出聚焦问题，不能展示公式、已知量、解题步骤或局部提示；完整解析只在支持阈值触发时展示。
...
2. 人工录入的全部 `guidedQuestions` 同样作为大模型上下文；非空时模型生成子问题应优先参考，未配置时根据其他题目材料和学生当前内容生成，不得把人工问题机械照抄为完整答案。
3. 教学模型结合后端提供的当前缺失评分点、已提供过的支持和规则约束，决定本次提示的具体内容和表达方式；第一次评价模型不读取这些历史或计数信息。
4. 系统不使用确定性代码判断“下一层提示”的语义，也不设置“人工提示已经耗尽”的独立业务分支。
5. 系统只决定本次提示是否允许发送、是否计数以及是否已经触发当前轮支持上限。
6. 大模型生成的局部提示不得直接泄露完整解析；达到支持上限时由系统展示人工录入的 `fullSolution`。
...
如果 round == 1 且 nextSupportCount >= firstRoundSupportLimit：
    ...
    如果学生选择“仍然不会”：
        申请人工复核
        保存具体原因，并回到 WAIT_STUDENT_ACTION
...
| 字段 | 含义 |
|---|---|
| `supportType` | `ASK_FOCUSED_QUESTION`、`GIVE_HINT`、`GIVE_CORRECTION` 或 `CORRECT_AND_ASK` |
| `round` | 发生轮次 |
| `status` | 当前固定为 `VALID` |
| `createdAt` | 发送时间 |
...
- 第一次评价调用因网络、超时、服务异常或结构校验重试耗尽：申请人工复核，保留已经确认的文本和失败审计，进入既有人工处理流程。
- 第二次教学调用发生提供商异常、结构解析失败或输出硬校验失败：不重试，保留已经持久化的合法评价，不创建 `SupportEvent`，不增加支持计数；会话直接进入 `status = NEED_HUMAN`、`flow_stage = WAIT_STUDENT_ACTION`，写入 `TEACHING_GENERATION_FAILED` 状态事件，并向本次请求返回 `TEACHING_GENERATION_FAILED` 错误。
- 基础设施失败不增加支持次数，不丢弃已经确认的输入；AI 服务失败会额外申请人工复核，避免单次错误阻断流程。
...
1. 必须填写反对理由。
2. 保存题目、确认文本、可用时的原 AI 结构化评价、被申诉的 AI 反馈和反对理由。
3. 不进行 AI 二次复核。
4. 不增加或撤销支持次数。
5. 申请人工复核并保持 `status = IN_PROGRESS`。
6. `needHumanReason` 记录学生申诉及其理由。
7. 自动流程继续，学生可以再次提交自讲。
8. 只要当前已有 AI 评价或支持回复，在 `IN_PROGRESS` 的所有稳定交互阶段（包括 `WAIT_GUIDED_ANSWERS`）均可提交申诉；已有子问题不阻断申诉。
...
人工复核申请是本阶段过程事件。系统只保存并报告具体原因，不实现人工派单、复核界面、人工处理或状态回写；申请后仍允许继续自讲。
...
| `IN_PROGRESS` | AI 无法可靠判断或建议人工处理 | `IN_PROGRESS` | 申请人工复核，保存具体原因并等待继续自讲 |
| `IN_PROGRESS` | AI 结构校验或服务重试耗尽 | `IN_PROGRESS` | 申请人工复核，保存审计信息并等待继续自讲 |
| `IN_PROGRESS` | 学生提交申诉理由 | `IN_PROGRESS` | 保存申诉证据，申请人工复核并等待继续自讲 |
| `IN_PROGRESS` | 第一轮看完解析后仍然不会 | `IN_PROGRESS` | 保存具体原因，申请人工复核并等待继续自讲 |
...
| `AI_EVALUATING` | AI 正在评价最终提交文本 | 否 |
| `WAIT_STUDENT_ACTION` | 已展示 AI 回复，等待下一步操作 | 是 |
| `WAIT_GUIDED_ANSWERS` | 等待学生逐题提交独立子问题答案 | 是 |
...
| 我有疑问 | `IN_PROGRESS` 的稳定交互阶段 | 可在自讲、子问题或完整解析过程中提交独立疑问；系统生成后续局部支持 |
| 我不同意 AI 判断 | 已展示 AI 评价或支持回复，且处于 `IN_PROGRESS` 的稳定交互阶段 | 要求填写理由，申请人工复核后继续自讲；已有子问题不阻断申诉 |
| 暂时退出 | 允许暂停的稳定阶段 | 保存上下文并进入 `PAUSED` |
| 看懂第一次完整解析 | 第一轮解析展示后 | 隐藏解析，重置当前轮统计，进入第二轮完整自讲 |
| 未看懂第一次完整解析 | 第一轮解析展示后 | 申请人工复核后继续自讲 |
...
1. 主输入框在整个非终态会话中保留学生最近的内容；学生可以在提示、子问题作答和每次反馈后继续修改。
2. “提交自讲”将当前主输入框保存为新的 `confirmedText` 快照，再进入正确性和完整性评价。历史快照不可覆盖。
3. “我不会”与“我有疑问”也将主输入框作为模型上下文，但空草稿不得创建 `ExplanationAttempt`，不得进入完整性评价。
4. 主输入框为空时，第一次点击“我不会”只提示“请先输入当前的题干理解或者解题、分析过程”；第二次允许进入提示流程。主输入框未更新时，第一次再次请求提示只提示“请先将刚才的思路补充到主输入框后再提交”；第二次允许进入提示流程。两类提醒均不调用模型、不增加任何计数。
5. 学生端输入区使用 Segmented 分段控制器展示“自讲”“回答子问题”“我有疑问”“AI说错了”四个输入块；切换分段不得清空各块已输入内容。该分段只改变页面承载方式，业务评价仍以 `confirmedText`、子问题答案、独立疑问文本和申诉理由等既有字段为准。

### 21.2 “我不会，给我一点提示”

1. 系统以本轮连续无进展的提示请求计数控制形式；该计数在出现新增评分点时清零。
2. 连续无进展的第 1、2 次请求，模型必须先给出 2～3 个子问题。子问题可以拆解当前问题，也可以询问必要概念或公式；发送这一组子问题即创建一条有效支持事件并计 1 次。
3. 学生可以逐题回答子问题；全部回答完成后，模型只补充错误或不完整子问题的答案，并给出整合引导。整合引导只能使用题目已给信息、主输入框已有信息和学生刚答对的信息，可以说明这些信息的使用顺序，不得新增公式、条件、关系或中间结论。
4. 子问题、子问题答案反馈和整合引导属于同一条支持事件，最多计 1 次，不因多条模型消息重复计数。
5. 连续无进展的第 3 次及以后请求，系统要求模型给出当前一个步骤的局部答案并计 1 次；若当前主输入框出现新增评分点，则清零连续无进展计数，仍采用子问题形式。
6. “无进展”先按确定性规则判断：主输入框与上次提示时的内容相同，即为无进展；内容变化时，模型返回的新增评分点为空，也为无进展。只有内容变化且存在新增评分点，才是有进展。
7. `GUIDED_QUESTION_REQUEST_LIMIT` 是配置项，当前值为 `2`；当前步骤答案在连续无进展请求次数大于该值时发送。

### 21.3 “我有疑问”

1. 学生输入独立疑问文本，系统同时将主输入框内容提交给模型。
2. 简单知识点疑问可直接回答，发送即计 1 次有效支持，但不改变连续无进展的“我不会”请求计数。
3. 关键步骤“具体怎么答”的疑问，模型必须转入 21.2 的子问题流程，并计入该流程的连续无进展请求计数。
4. 学生不知道疑问点时，可选择“我不知道卡在哪里，给点提示”，直接走 21.2。
5. 学生直接要求完整答案时，模型只拒绝并引导其回到当前过程和局部问题；拒绝不计有效支持，不触发完整解析。

### 21.4 有效支持、阈值与第二轮

1. 实际发送的子问题组、纠错、纠错后追问、简单知识点答复和当前步骤答案按既有规则计有效支持；评价产生的聚焦追问不计有效支持。一个 `CORRECT_AND_ASK` 即使同时包含纠错和一个子问题，也只计一次。
2. 评价结果展示、第一次提醒、完整答案请求拒绝、基础设施失败、结构校验失败和完整解析展示不计有效支持。
3. 触发 6/3 阈值时，在发送新的教学性内容之前展示完整解析；不创建本次阈值支持事件，也不增加 `supportCountTotal`。
4. 第一轮只有在展示完整解析后才进入第二轮。第一轮未展示完整解析时，无论已获得 0、1 或 2～5 次有效支持，只要自讲正确且完整即直接 `COMPLETED`。

### 21.5 数据契约与审计

会话增加 `currentDraft`、`lastSupportDraft` 和 `noProgressHelpRequestCount`。支持事件除原有字段外，保存 `supportKind`、主草稿、疑问文本、子问题、子问题答案和答案反馈；完整答案拒绝保存为不计数事件。所有状态、草稿比较、进展计数、阈值判断和支持计数均由确定性代码执行。

学生可见的“学习反馈记录”调整为“自讲记录”。自讲记录按对话形式展示学生每次提交的内容以及 AI 或系统后续反馈，记录区域使用滚动展示；学生端仍不得展示原始结构化输出、审计字段或模型校验细节。

题目增加可选的 `guidedQuestions`。未配置时保存为空列表；录题人员一旦添加提示子问题，空白项和重复项均被拒绝。题目创建、编辑和详情读取均返回该字段。`guidedQuestions` 非空时作为 AI 评价、子问题生成和子问题作答评估的优先参考上下文；为空时模型根据其他题目材料和学生当前内容生成子问题，不新增状态或计数分支。
