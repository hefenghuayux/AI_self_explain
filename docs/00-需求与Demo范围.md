...
| `guidedQuestions` | 人工录入、供模型组织子问题提示时优先参考的问题列表 |
| `fullSolution` | 完整解析 |
...
2. 人工录入的全部 `guidedQuestions` 同样作为大模型上下文；模型生成子问题时优先参考它们，不得把它们机械照抄为完整答案。
...
题目增加必填的 `guidedQuestions`。录题人员逐条录入提示子问题，空项和重复项均被拒绝；题目创建、编辑和详情读取均返回该字段。已有历史题目迁移后保存为空列表，重新编辑时必须补录。`guidedQuestions` 作为 AI 评价、子问题生成和子问题作答评估的上下文，不参与确定性评分点或计数判断。
